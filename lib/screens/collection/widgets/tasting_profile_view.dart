...
import 'package:steapleaf/core/models/flavor_profile.dart';

import '../../../theme/steapleaf_theme.dart';
import 'spider_chart.dart';

/// Read-only-Anzeige eines [FlavorProfile] mit zwei Spider-Charts
class FlavorProfileView extends StatelessWidget {
  final FlavorProfile profile;

  const FlavorProfileView({super.key, required this.profile});
...
class _TextTag extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DecoratedBox(
      ...
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: SteapLeafSpacing.xs, vertical: 4),
        child: Text(
          label,
          style: textTheme.bodySmall?.copyWith(
            color: colorScheme.onSurfaceVariant,
